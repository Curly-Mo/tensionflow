...
class Dataset(object):
    def __init__(self, filepath=None, splits=['training', 'test', 'validation'], preprocessor=None, indexify_labels=True):
        self.meta = {}
        self.splits = {}
        ...
        def gen(X, Y):
            for x, y in zip(X, Y):
                try:
                    ...
                    self.meta['data_struct'] = tuple(parse_data_structure(new, old) for new, old in itertools.zip_longest(data, self.meta['data_struct']))
                    yield data
    ...
    def load(self, filepath, split='training', dtypes={'labels': tf.int64, 'features': tf.float32}):
...
def parse_data_structure(array, prev_struct=None):
    ...
    if prev_struct:
        if prev_struct['dtype'] != dtype:
            logger.warn(f'dtype {dtype} does not match previous dtype: {prev_struct["dtype"]}')
